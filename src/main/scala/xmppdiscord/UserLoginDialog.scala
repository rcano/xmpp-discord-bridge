...
class UserLoginDialog(user: String, connectedClient: () => XmppClient) extends Dialog[(String, XmppClient)] {
  ...
  private val tokenTextField = new PasswordField().tap { tf => tf setPromptText "password"; tf.setMaxWidth(Double.MaxValue); tf setPrefColumnCount 30 }
  ...
  loginButton.addEventFilter[ActionEvent](ActionEvent.ACTION, evt => {
      ...
      if (!tokenTextField.getText.isEmpty) {
        ...
        val logingIn = Future {
          val xmppClient = connectedClient()
